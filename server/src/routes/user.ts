...
router.get('/sync', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    if (error || !user) return res.status(401).json({ error: 'Session invalide — utilisateur introuvable' });
    ...
    if (!hasPiyes) {
      ...
      accounts.sort((a: any, b: any) => (a.provider === 'piyes' ? -1 : b.provider === 'piyes' ? 1 : 0));
    }
...
router.post('/notifications/mark-read', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    let query = supabase.from('Notification').update({ isRead: true }).eq('userId', userId);
...
router.post('/profile', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    const { 
      name, tag, email, phone, dob, address, 
      nationality, idNumber, language, timezone,
      avatarUrl, otpCode
    } = req.body;
    ...
    const initials = name ? name.trim().split(/\s+/).map((n: string) => n[0]).join('').substring(0, 2).toUpperCase() : undefined;
...
router.get('/search', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    let dbQuery = supabase.from('User').select('id, name, tag, phone, avatarUrl, initials, verificationStatus');
    ...
    const filteredUsers = (users || [])
      ...
      .map(u => ({
        ...u,
        avatarUrl: u.avatarUrl ? `${u.avatarUrl}${u.avatarUrl.includes('?') ? '&' : '?'}t=${Date.now()}` : null
      }));
...
router.get('/keys/check-tag', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    // Check Key table
    const { data: existingKey } = await supabase.from('Key').select('id').ilike('value', tagWithAt).maybeSingle();
    if (existingKey) return res.json({ available: false });
    
    // Check User table
    const { data: existingUser } = await supabase.from('User').select('id').ilike('tag', tagWithAt).maybeSingle();
...
router.post('/keys', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    if (type === 'tag') {
      if (!cleanValue.startsWith('@') || !/^[a-z0-9_]{4,25}$/.test(cleanValue.substring(1).toLowerCase())) {
        return res.status(400).json({ error: 'Invalid tag format (@username, 4-25 chars, a-z0-9_)' });
      }
    } else if (type === 'email') {
      ...
      if (!/^\+509\d{8}$/.test(cleanValue)) {
        return res.status(400).json({ error: 'Invalid phone format (must be +509 followed by 8 digits)' });
      }
    }
    ...
    if (type === 'tag') {
      const { data: existingUserTag } = await supabase.from('User').select('id').ilike('tag', cleanValue).maybeSingle();
      if (existingUserTag) return res.status(400).json({ error: 'Tag already in use' });
    }
...
router.post('/keys/:id/verify', authMiddleware, async (req: AuthRequest, res) => {
  try {
    const userId = req.user?.id;
    if (!userId) return res.status(401).json({ error: 'Unauthorized' });
    ...
    if (isValid) {
      ...
      if (!metadata) {
        ...
        const { data: existingKey } = await supabase.from('Key').select('id, isVerified').eq('id', id).eq('userId', userId).maybeSingle();
    ...
    return res.status(400).json({ error: 'Verification failed. Please try adding the key again.' });
  } catch (error) {
...
router.get('/qr', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    const qrData = JSON.stringify({
      ...
      phone: user.phone,
      email: user.email,
      avatarUrl: user.avatarUrl ? `${user.avatarUrl}${user.avatarUrl.includes('?') ? '&' : '?'}t=${Date.now()}` : null
    });
...
router.patch('/contact-update/:contactId', authMiddleware, async (req: AuthRequest, res) => {
  try {
    const userId = req.user?.id;
    if (!userId) return res.status(401).json({ error: 'Unauthorized' });
    ...
    const { data, error } = await supabase
      .from('Contact')
      .update({ name, tag, phone, email, randomKey, updatedAt: new Date().toISOString() })
      .eq('id', contactId)
      .eq('userId', userId)
      .select()
      .single();
