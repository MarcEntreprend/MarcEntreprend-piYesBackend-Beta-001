...
const detectKeyType = (
  info: string,
): { tag?: string; email?: string; phone?: string; randomKey?: string } => {
  ...
  try {
    ...
    const syncedContacts = await Promise.all(
      contacts.map(async (c) => {
        ...
        const rawKey =
          c.info || c.tag || c.email || c.phone || c.randomKey || "";
        ...
        if (resolvedTag) {
          const { data } = await supabase
            ...
            .ilike(
              "tag",
              resolvedTag.startsWith("@") ? resolvedTag : `@${resolvedTag}`,
            )
            .maybeSingle();
          contactUser = data;
        }
        ...
        if (contactUser?.id) {
          const { data } = await supabase
            ...
            .select("id")
            .eq("userId", userId)
            .eq("contactUserId", contactUser.id)
            .maybeSingle();
          existing = data;
        }
        ...
        if (!existing) {
          ...
          if (resolvedRandomKey)
            orConditions.push(`randomKey.eq.${resolvedRandomKey}`);
    ...
    if (keyToCheck) {
      if (tag) {
        const { data } = await supabase
          ...
          .ilike("tag", tag.startsWith("@") ? tag : `@${tag}`)
          .maybeSingle();
        contactUser = data;
      } else if (email) {
        const { data } = await supabase
