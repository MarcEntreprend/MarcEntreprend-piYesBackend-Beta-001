...
import { transferSchema, rechargeSchema, requestPaymentSchema, schedulePaymentSchema, depositWithdrawSchema, interBankTransferSchema } from '../../../shared/schemas';
...
router.post('/transfer', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    const { data: userReceiver } = await supabase
      .from('User')
      .select('*')
      .or(`id.eq."${validated.contactId}",tag.ilike."${validated.contactId}",accountNumber.eq."${validated.contactId}"`)
      .maybeSingle();
    ...
    if (!receiverPermAccount || receiverPermAccount.permission !== 'oui') {
      throw new Error('Ce destinataire ne peut pas recevoir de paiements pour le moment');
    }
    ...
    console.log(`Processing transfer: sender=${sender.id}, receiver=${receiver.id}, pin_provided=${!!validated.pin}`);
    ...
    if (txError2) console.error('Failed to create receiver transaction record:', txError2);

    // Update balances
    const { error: decError } = await supabase
      .from('User')
      .update({ balance: sender.balance - amountCents, updatedAt: new Date().toISOString() })
      .eq('id', sender.id);
    ...
    const { error: incError } = await supabase
      .from('User')
      .update({ balance: receiver.balance + amountCents, updatedAt: new Date().toISOString() })
      .eq('id', receiver.id);
    if (incError) {
    ...
    if (existingContact) {
      await supabase.from('Contact').update(contactData).eq('id', existingContact.id);
    } else {
    ...
    if (receiverExistingContact) {
      await supabase.from('Contact').update(receiverContactData).eq('id', receiverExistingContact.id);
    } else {
      await supabase.from('Contact').insert({
        ...receiverContactData,
        id: crypto.randomUUID(),
    ...
    const recipientInitials = nameParts.length > 1 
    ...
    res.status(400).json({ error: { message: error.message || 'Transfer failed' } });
  }
});
...
router.post('/recharge', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    const { error: updateAccError } = await supabase
      .from('Account')
      .update({ balance: newAccountBalance, updatedAt: new Date().toISOString() })
      .eq('id', account.id);
    ...
    res.status(400).json({ error: { message: error.message || 'Recharge failed' } });
  }
});
...
router.post('/deposit', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    const { data: user } = await supabase.from('User').select('balance').eq('id', userId).single();
    ...
    if (validated.accountId) {
      ...
      if (sourceAccount?.provider === 'moncash') {
        try {
          ...
          const redirectUrl = await moncashService.createPayment(validated.amount, orderId);
          ...
          return res.status(400).json({ error: { message: error.message || 'MonCash initialization failed' } });
        }
      }
    }
    ...
    await supabase.from('User').update({ balance: newBalance }).eq('id', userId);
    await supabase.from('Account').update({ balance: newBalance }).eq('userId', userId).eq('provider', 'piyes');
    ...
    res.status(400).json({ error: { message: error.message || 'Deposit failed' } });
  }
});
...
router.post('/withdraw', authMiddleware, async (req: AuthRequest, res) => {
  try {
      ...
      if (destAccount?.provider === 'moncash') {
        try {
          ...
          if (merchantBalance < validated.amount) {
            throw new Error('Service temporairement indisponible (Solde marchand insuffisant)');
          }

          // 2. Execute transfer
          const reference = generateId();
          const result = await moncashService.transfer(validated.amount, destAccount.accountNumber, reference);
          ...
          if (error.message === 'Maximum Account Balance') {
            return res.status(400).json({ 
              error: { 
                message: "Désolé, votre compte MonCash a atteint son plafond légal. Veuillez vider votre compte MonCash avant de demander un retrait piYès.",
                code: 'MONCASH_LIMIT_REACHED'
              } 
            });
          }
          return res.status(400).json({ error: { message: error.message || 'MonCash withdrawal failed' } });
        }
      }
    }
    ...
    res.status(400).json({ error: { message: error.message || 'Withdraw failed' } });
  }
});
...
router.post('/request', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    const { data: sender } = await supabase.from('User').select('name,id').eq('id', userId).single();
    ...
    const { data: user } = await supabase.from('User').select('tag, phone, email').eq('id', userId).single();
    const to = user?.tag?.replace('@', '') || user?.phone || user?.email || userId;
    const type = user?.tag ? 'tag' : user?.phone ? 'phone' : user?.email ? 'email' : 'id';
    const amount = validated.amount;
    const paymentLink = `https://piyes.ht/pay?to=${encodeURIComponent(to)}&type=${type}&amount=${amount}`;
    
    res.json({ success: true, message: 'Payment request created', paymentLink });
  } catch (error: any) {
    res.status(400).json({ error: { message: error.message || 'Request failed' } });
  }
});
...
router.post('/schedule', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    res.status(400).json({ error: { message: error.message || 'Scheduling failed' } });
  }
});
...
router.post('/scan', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    const { data: sender } = await supabase.from('User').select('*').eq('id', userId).single();
    if (!sender) throw new Error('User not found');
    ...
    // 1. Try by ID
    if (receiverId) {
      const { data } = await supabase.from('User').select('*').eq('id', receiverId).maybeSingle();
      receiver = data;
    }
    
    // 2. Try by Tag
    if (!receiver && receiverTag) {
      const { data } = await supabase.from('User').select('*').eq('tag', receiverTag).maybeSingle();
      receiver = data;
    }

    // 3. Try by Phone
    if (!receiver && receiverPhone) {
      const { data } = await supabase.from('User').select('*').eq('phone', receiverPhone).maybeSingle();
      receiver = data;
    }

    // 4. Try by Email
    if (!receiver && receiverEmail) {
      const { data } = await supabase.from('User').select('*').eq('email', receiverEmail).maybeSingle();
      receiver = data;
    }
    ...
    await supabase.from('User').update({ balance: newSenderBalance }).eq('id', sender.id);
    await supabase.from('Account').update({ balance: newSenderBalance }).eq('userId', sender.id).eq('provider', 'piyes');
    
    await supabase.from('User').update({ balance: newReceiverBalance }).eq('id', receiver.id);
    await supabase.from('Account').update({ balance: newReceiverBalance }).eq('userId', receiver.id).eq('provider', 'piyes');
    ...
    res.status(400).json({ error: { message: error.message || 'QR Payment failed' } });
  }
});
...
router.get('/', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    res.json((transactions || []).map((tx: any) => ({ ...tx, amount: tx.amount / 100 })));
  } catch (error) {
...
router.get('/receipts/:id', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    const senderAccount = tx.role === TransactionRole.PAYER ? myAccount : counterpartyAccount;
    const senderUser = tx.role === TransactionRole.PAYER ? myUser : counterpartyUser;
    
    const receiverAccount = tx.role === TransactionRole.RECEIVER ? myAccount : counterpartyAccount;
    const receiverUser = tx.role === TransactionRole.RECEIVER ? myUser : counterpartyUser;
    ...
    const receipt = {
      ...
      sender: {
        name: senderUser?.name || (tx.role === TransactionRole.PAYER ? 'Moi' : tx.counterpartyName),
        masked_account: senderAccount?.accountNumber ? maskAccountNumber(senderAccount.accountNumber) : undefined,
        idNumber: senderUser?.idNumber,
        bank: senderAccount?.provider === 'piyes' ? 'piYès' : (senderAccount?.label || 'piYès')
      },
      receiver: {
        name: receiverUser?.name || (tx.role === TransactionRole.RECEIVER ? 'Moi' : tx.counterpartyName),
        masked_account: receiverAccount?.accountNumber ? maskAccountNumber(receiverAccount.accountNumber) : undefined,
        idNumber: receiverUser?.idNumber,
        bank: receiverAccount?.provider === 'piyes' ? 'piYès' : (receiverAccount?.label || 'piYès')
      }
    };
...
router.post('/inter-bank-transfer', authMiddleware, async (req: AuthRequest, res) => {
  try {
    const userId = req.user?.id;
    if (!userId) return res.status(401).json({ error: 'Unauthorized' });
    ...
    if (validated.sourceId === 'piyes-main') {
      const { data } = await supabase.from('Account').select('*').eq('userId', userId).eq('provider', 'piyes').maybeSingle();
      if (data) {
        sourceAcc = data;
      } else {
      ...
      const { data } = await supabase.from('Account').select('*').eq('id', validated.sourceId).eq('userId', userId).single();
      sourceAcc = data;
    }
    ...
    if (validated.destId === 'piyes-main') {
      ...
      const { data } = await supabase.from('Account').select('*').eq('id', validated.destId).eq('userId', userId).single();
      destAcc = data;
    }
    ...
    if (destAcc.provider !== 'moncash') {
      const { error: destUpdateError } = await supabase
        ...
        .eq('id', destAcc.id);
      if (destUpdateError) {
        console.error('Failed to update destination account balance:', destUpdateError);
    ...
    const { data: payerTx, error: payerTxError } = await supabase
      ...
      .insert({
        ...
        type: TransactionType.TRANSFER,
        amount: amountCents,
        description: validated.note || `Transfert inter-bancaire: ${sourceAcc.label} -> ${destAcc.label}`,
        role: TransactionRole.PAYER,
...
router.post('/moncash/confirm', authMiddleware, async (req: AuthRequest, res) => {
  try {
    const userId = req.user?.id;
    if (!userId) return res.status(401).json({ error: 'Unauthorized' });
    ...
    const { moncashService } = await import('../services/moncashService');
    const result = await moncashService.retrieveTransactionPayment(transactionId);
    ...
    if (result.payment.message === 'successful') {
      ...
      let txId = generateId();
      if (orderId) {
        const { data: existingTx } = await supabase.from('Transaction').select('id').eq('id', orderId).single();
      ...
      return res.status(400).json({ error: 'Payment failed or not successful' });
    }
  } catch (error: any) {
    console.error('MonCash confirmation error:', error);
    res.status(400).json({ error: { message: error.message || 'Confirmation failed' } });
  }
});
...
router.get('/reports', authMiddleware, async (req: AuthRequest, res) => {
  try {
   ...
   let txs: any[] = [];
    ...
    const prevReceived = prevTxs.filter(t => t.role === 'RECEIVER').reduce((s, t) => s + t.amount, 0) / 100;
    const prevSent = prevTxs.filter(t => t.role === 'PAYER').reduce((s, t) => s + t.amount, 0) / 100;

    // Top 5 payeurs (counterpartyName des transactions reçues)
    const senderMap = new Map<string, { amount: number, count: number }>();
    received.forEach(t => {
      const existing = senderMap.get(t.counterpartyName) || { amount: 0, count: 0 };
    ...
    const byHour = Array.from({ length: 24 }, (_, hour) => {
      const hourTxs = received.filter(t => new Date(t.date).getHours() === hour);
      return {
        hour,
    ...
    res.json({
      ...
      byHour,
      byType,
      avgTransactionAmount: txs.length > 0 ? (totalReceived + totalSent) / txs.length : 0,
      totalFeesPaid,
      frequencyBreakdown
    });
  } catch (error) {
...
router.post('/international', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    const { amount, country, recipientName, method, methodInfo, currency, amountForeign, exchangeRate } = req.body;

    if (!amount || !country || !recipientName || !method) {
      return res.status(400).json({ error: { message: 'Champs obligatoires manquants', code: 'MISSING_FIELDS' } });
    }
    ...
    if (user.balance < amountCents) {
      return res.status(400).json({ error: { message: 'Solde insuffisant', code: 'INSUFFICIENT_BALANCE' } });
    }
    ...
    await supabase.from('Account')
      ...
      .eq('userId', userId).eq('provider', 'piyes');
    ...
    res.status(400).json({ error: { message: error.message || 'Transfert international échoué' } });
  }
});
...
router.get('/resolve/:key', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    if (!receiver) {
      return res.status(404).json({ error: { message: 'Destinataire introuvable', code: 'NOT_FOUND' } });
    }
    ...
    if (receiverAccount && receiverAccount.permission !== null && receiverAccount.permission !== 'oui') {
      return res.status(403).json({
        error: { message: 'Ce destinataire ne peut pas recevoir de paiements pour le moment', code: 'PERMISSION_DENIED' }
      });
    }
