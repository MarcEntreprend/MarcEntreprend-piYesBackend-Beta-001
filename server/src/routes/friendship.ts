...
router.post("/request", authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    if (!contactUserId)
      return res.status(400).json({ error: "contactUserId requis" });
        ...
        await supabase.from("Notification").insert([
          {
            ...
            body: `${requester?.name} a accepté votre demande d\'ami.`,
            isRead: false,
            route: "/contacts",
            targetId: requesterId,
            timestamp: now,
          },
          {
            ...
            body: `Vous êtes maintenant amis avec ${receiver?.name}.`,
            isRead: false,
            route: "/contacts",
    ...
    const { data: requester } = await supabase
      .from("User")
      .select("name, tag, phone, email, avatarUrl")
      .eq("id", requesterId)
      .single();
...
router.delete("/cancel", authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    const { error } = await supabase
      ...
      .delete()
      .or(
        `and(requesterId.eq.${userId},receiverId.eq.${contactUserId}),and(requesterId.eq.${contactUserId},receiverId.eq.${userId})`,
      );
